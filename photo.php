		...
		<div class="buy_votes_pop_inner">
			...
			<div class="buying_votes_box">
				...
				// print_r($fetch_items);
					...
					data-vote="'.$fetch_items['votes'].'"
					data-views="'.$fetch_items['views'].'"

					data-photo_name_="'.$fetch_photo['photo'].'"
					data-photo_id="'.$fetch_photo['id'].'"
					>
						<span class="itemlist_name">'.$fetch_items['item_name']. ' -' .$fetch_items['views'].' Views</span><br>
		...
		<div class="celling">
			...
			<div class="overflow">
				...
				<div class="photo_page_left overflow">
					...
					<div class="photo_page_info photo_order_7">
						...
						<div class="photo_comments_box">
							...
							<div class="photo_comments_add">
								<div class="overflow">
									<div class="photo_comments_form_left <?=(!is_logged() ? 'click_notlogged':'');?>">
										...
										<?php } else { ?>
										<input type="text" class="add_comment_input asad" name="add_comment" maxlength="200" placeholder="<?=_LANG_ADD_COMMENT;?>" />
										<?php } ?>
									</div>
									<div class="add_comment add_comment_for_admin"><i class="fas fa-check"></i></div>
								</div>
							</div>
							...
							<div class="message" style="background:#000; color:#fff; text-align: center; padding:15px; display: none;"></div>
				...
				<div class="photo_tab_2 <?=(isset($settings['photo_sidebar']) && $settings['photo_sidebar'] == '1' ? 'left':'');?> relative photo_order_3 overflow">
						...
						<div class="photo_rating_action">
							...
							<div class="profile_ratings photo_rating_action_box">
								...
								if(isset($settings['content_ratemode']) && $settings['content_ratemode'] == '1') {
								 ...
								 if($site_contest['active'] == '0' || ($site_contest['active'] == '1' && strtotime($site_contest['end']) > time())) { ?>
									...
									<div class="profile_rating_left2 <?=($i_rated == '0' ? 'rate_active':'rate_inactive');?> extra_rating_s <?=(isset($settings['vote_own']) && $settings['vote_own'] == '0' && is_logged() && isset($_SESSION['_logged_id']) && $_SESSION['_logged_id'] == $fetch_photo['iduser'] ? 'hide':'');?>">
										<?php if($i_rated == '0') { ?>
											<div class="vote_button_click" data-id="<?php echo $fetch_photo['rating']; ?>"><?=_LANG_VOTE_BUTTON;?></div>
								...
								<div class="photo_list_board_item photo_list_board_item_current">
									...
									<div class="photo_list_board_item_val nr_rating_update current_score_val">
										<?=$fetch_photo['4'];?></div>
								</div>
					...
					if($total_items && isset($_SESSION['_logged_id'])) { ?>
						<!-- Buy votes button div -->
