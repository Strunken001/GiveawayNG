(function ($) {
	...
	$(document).on('click', '.buy_contest_click', function () {
		...
		$.post('_core/request.php',
			{ reason: 'paynow_contest', item_id: id }
			, function (get) {

				if (get.error == '0') {


					if (get.ptype == 'paystack') {

						loadExternalScript('https://js.paystack.co/v1/inline.js').then(function () {

							var handler = PaystackPop.setup({
								key: get.paystack_public_key,
								...
								metadata: {
									custom_fields: [
										{
											display_name: get.user_name,
											variable_name: get.user_email
										}
									]
								},
								callback: function (response) {
									...
									$('.unpaid_contest').html('Payment done. Contest will approve in some time');
									$('.unpaid_contest').css('background', 'green');
								},
								onClose: function () {
									alert('window closed');
								}
	...
	$(document).on('click', '.paynow_joincontest', function () {
		...
		$.ajax({
			url: '_core/request.php',
			type: 'POST',
			crossDomain: true,  // Allow cross-domain requests
			xhrFields: {
				withCredentials: true  // Include credentials in the request (if needed)
			},
			data: { reason: 'paynow_joincontest', item_id: id },
			success: function (get) {
				if (get.error == '0') {
					// Your existing code for payment options
					if (get.ptype == 'paystack') {

						loadExternalScript('https://js.paystack.co/v2/inline.js').then(function () {
							...
							paystack.newTransaction({
								...
								onSuccess: (transaction) => {
									...
									//console.log('returnUrl:'+get.return_url + '?unid=' + get.unid + '&contest_id=' + id+'&ptype=3');
									window.location = get.return_url + '?unid=' + get.unid + '&contest_id=' + id +'&ptype=2';
								},
								onCancel: () => {
									// user closed popup
									alert('window closed');
					...
					else if (get.ptype == 'paypal') {
						// PayPal payment code
					} else if (get.ptype == 'razorpay') {
	...
	$(document).on('click', '.buy_item_click', function () {
		...
		var id = $(this).data('id');
		var photo_name_ = $(this).data('photo_name_');
		var votes = $(this).data('vote');
					...
					loadExternalScript('https://js.paystack.co/v1/inline.js').then(function () {
						...
						var handler = PaystackPop.setup({
							...
							callback: function (response) {
								...
								// return false;
								// alert(get.return_url);return
								window.location = get.return_url + '?unid=' + get.unid + '&ptype=2&photo_id=' + photo_id;

								$('.buy_votes_click').html('Payment done');
								$('.buy_votes_click').css('background', 'green');
							},
							onClose: function () {
								alert('window closed');
							}
	...
	$(document).on('click', '.add_comment', function () {
		...
		if (comment.length) {
			...
			$.post('_core/request.php', { reason: 'add_comment', comment: comment, photo_id, photo_id }, function (get) {
				...
				if (get.error == '0') {
					// alert('abc');
					if (get.added_by == 'user') {
						$('.message').css('display', 'block');
						$('.message').html('Comment under review');
					} else {
						location.reload();
					}
				} else {
	...
	if (this_page == 'contest' && active_contest == '1') {
		...
		var x = setInterval(function () {
			...
			// Set the time zone to Nigeria (West Africa Time)
			var options = {
				...
				var now2 = new Date().toLocaleString('en-US', options);
	...
	if (this_page == 'completed_contests') {
	...
	if (logged_id == 0) {
		...
		console.log('logged id' + logged_id);
	}
	...
	$(document).on('click', '.click_register', function () {
		...
		var fullPhoneNumber = selectedCountryCode + phoneNumber;
		...
		$.post('_core/request.php', { email: email, name: name, fullPhoneNumber: fullPhoneNumber, password: password, repeat_password: repeat_password, category: category, reason: 'register' }, function (get) {

			if (get.error == '0') {
				if (email_confirm == '1') {
					$('.open_pop[data-id="login"]').click();
					$('.click_register').css('background', '#00cc00').html('<i class="fas fa-spinner fa-spin"></i>');
