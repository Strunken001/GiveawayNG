...
session_start();
	...
	if($reason == 'contests') {
		...
		while($fetch_contest = mysqli_fetch_array($sql_contests)) {
			...
			if(time() > $countDownDate) {
				...
				mysqli_query($db,"UPDATE `contest` SET `active` = 0 WHERE `id` = '".$fetch_contest['id']."' LIMIT 1");

			} else {
				date_default_timezone_set('Africa/Lagos');
				$currentDateTime = new DateTime();
				$endDateTime = new DateTime($fetch_contest['end']);
				// Set time zone for both DateTime objects
				$endDateTime->setTimezone(new DateTimeZone('Africa/Lagos'));
				$currentDateTime->setTimezone(new DateTimeZone('Africa/Lagos'));
				// Calculate the difference between end and current datetime
			...
			$return['role_id']=$_SESSION['_logged_role'];
		...
		if(mysqli_num_rows($sql)) {
			...
			if(!mysqli_num_rows($sql2)) {
				...
				if(!mysqli_num_rows($sql3)) {
					mysqli_query($db,"UPDATE `users` SET `role_id` =$role_id,  `mobile_number` = '".$mobile_number."',`email` = '".$email."', `name` = '".$name."', `user` = '".$user."' WHERE `id` = '".$user_id."' LIMIT 1");
					$return['error'] = 0; 
				} else {
					$return['error'] = 2;
				}
	...
	if($reason == 'get_edit_user' && isset($_POST['id'])) {
		...
		if(mysqli_num_rows($sql)) {
			...
			$return['mobile_number'] = $fetch['mobile_number'];
			$return['user'] = $fetch['user'];
			$return['role_id'] = $fetch['role_id'];
			$return['error'] = 0;
		}

	}

	if($reason == 'export_users') {

		$query = "SELECT id, email ,mobile_number,name,user,registered FROM users";
    ...
    if ($result->num_rows > 0) {
        ...
        fputcsv($output, array('ID', 'Email','Mobile','Name','UserName','Registeration'));
        ...
        echo json_encode(['success' => true, 'file_content' => ob_get_clean()]);
		ob_end_clean(); // Clear the output buffer to prevent issues with JSON encoding
    } else {
	...
	if($reason == 'users') {
		...
		while($fetch_user = mysqli_fetch_array($sql_users)) {
			...
			$return['users'][] = array(
				...
				'mobile_number' =>$fetch_user['mobile_number'],
	...
	if($reason == 'payments') {
		...
		while($fetch_payment = mysqli_fetch_array($sql_payments)) {
			...
			if(mysqli_num_rows($sql_item)) {
				...
				$sql_item = mysqli_query($db,"SELECT * FROM `contest` WHERE `id`= '".$fetch_payment['item_number']."' LIMIT 1");
