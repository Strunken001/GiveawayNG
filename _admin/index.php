...
if (!isset($_SESSION['_logged_admin']) || $_SESSION['_logged_role']>=3) {
	header('location: login.php');
	die();
}
...
if (isset($_GET['page']) && $_GET['page'] == 'contest') {
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if (!$title) {
			...
			$createContestResponse['status'] = false;
			$createContestResponse['errors'][] = $contest_error_msg;
		}
		if (!$description) {
			$contest_error_msg = 'Description is required';
			$createContestResponse['status'] = false;
			$createContestResponse['errors'][] = $contest_error_msg;
		}
		if ($end == '0000-00-00T00:00') {
			$contest_error_msg = 'Finish Date is required';
			$createContestResponse['status'] = false;
			$createContestResponse['errors'][] = $contest_error_msg;
		}
		...
		if (!$contest_error_msg) {
			...
			if (isset($extra_id) && $extra_id != '0') {
				mysqli_query($db, "UPDATE `contest` SET `title` = '" . $title . "', `description` = '" . $description . "', `active` = '" . $active . "', `end` = '" . $end . "', `disable_countdown` = '" . $disable_countdown . "', `entry_fee` = '" . $entry_fee . "' WHERE `id` = '" . $extra_id . "' LIMIT 1");
			} else {
				mysqli_query($db, "INSERT INTO `contest` (`active`,`title`,`description`,`end`,`disable_countdown`,`entry_fee`) VALUES ('" . $active . "','" . $title . "','" . $description . "','" . $end . "','" . $disable_countdown . "','".$entry_fee."')") or die(mysqli_error($db));
				$extra_id = mysqli_insert_id($db);
				/* Sending email and sms 
					alert start */
				// Fetch name, email, and phone_number of all users
				$userDetailsQuery = "SELECT name, email, mobile_number FROM users";
				$userDetailsResult = $db->query($userDetailsQuery);

				if ($userDetailsResult) {
					// Fetch contest details
					$contestDetailsQuery = "SELECT * FROM contest WHERE id = $extra_id";
					$contestDetailsResult = $db->query($contestDetailsQuery);

					if ($contestDetailsResult && $contestDetailsResult->num_rows > 0) {
						$contestDetails = $contestDetailsResult->fetch_assoc();

						// Prepare email details
						$subject = 'New Contest Created: ' . $contestDetails['title'];
						$title = $contestDetails['title'];
						$desc = $contestDetails['description'];
						$expDate = $contestDetails['end'];
						$prize = $_POST['prize_name'][0];
						

						// Initialize an array to store phone numbers for SMS
						$phoneNumbersForSMS = array();
						try {
							// Prepare email details
							$emailDetails = array(
								'subject' => $subject,
							...
							while ($user = $userDetailsResult->fetch_assoc()) {
								// Prepare email details
								$name = $user['name'];
								$toAddresses[] = array(
									'email' => $user['email'],
								...
								<body>
									<div class='email-container'>
										...
										<div class='message'>
										A new giveaway contest has been created on GiveawayNG - Nigeria’s first giveaway
										<br>
										 platform! Hurry up and join to increase your chances of winning. See details below:
											<br>
											Title:$title
											<br>
											Description:$desc
											...
											Prize:$prize
											<br>
											Visit now at https://giveawayng.com
								...
								sendEmail($user['email'], $name, $subject, $emailMessage);
								// Add phone number to the array for SMS
								$phoneNumbersForSMS[] = $user['mobile_number'];
							}
							

							// Filter out empty numbers
							$nonEmptyNumbers = array_filter($phoneNumbersForSMS);
						} catch (Exception $e) {
							// Log any exceptions
							$logFileName = __DIR__ . '/log/' . date('Y-m-d_H:i:s') . '_email_log.txt';
							file_put_contents($logFileName, "Error: " . $e->getMessage() . PHP_EOL, FILE_APPEND);
						}
						try {
							if (!empty($nonEmptyNumbers)) {
								$url = "https://smsc.bullionsms.com/smsAPI";
								$logFileName = __DIR__ . '/log/' . date('Y-m-d_H:i:s') . '_sms_log.txt';
								// Set query parameters as variables
								$api_key = "9eNVFWgWjgsmBAF9K9rjsB73wM6TWLkM";
								$api_token = "z0St1585883185";
								$type = "sms";
								$from = "GiveawayNG";
								$to = implode(',', $nonEmptyNumbers);
								$text ="A new contest has been created on GiveawayNG. Hurry and join at https://giveawayng.com";
								$route = 0;
								...
								$smsApiResponse = curl_exec($ch);

								// Check for cURL errors
								if (curl_errno($ch)) {
									file_put_contents($logFileName, "Curl error: " . curl_error($ch) . ". " . PHP_EOL, FILE_APPEND);
								}

								curl_close($ch);
								// Decode the SMS API response
								$smsApiResponseDecoded = json_decode($smsApiResponse, true);
								// Log the SMS API response
								file_put_contents($logFileName, $smsApiResponse . PHP_EOL, FILE_APPEND);

								if ($smsApiResponseDecoded['status'] === 'queued' && isset($smsApiResponseDecoded['group_id'])) {
									$groupId = $smsApiResponseDecoded['group_id'];
									// Make another POST request to check SMS status using the group ID
									$urlCheckStatus = "https://smsc.bullionsms.com/smsAPI";

									// Set query parameters for checking SMS status
									$query_params_check_status = http_build_query([
										'groupstatus' => '',
										'apikey' => $api_key,
										'apitoken' => $api_token,
										'groupid' => $groupId,
									...
									// Include query string in the URL for checking SMS status
									$urlCheckStatus .= '?' . $query_params_check_status;

									$ch = curl_init();

									// Set cURL options for POST request to check SMS status
									curl_setopt($ch, CURLOPT_URL, $urlCheckStatus);
									curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
									curl_setopt($ch, CURLOPT_POST, true);
									curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);

									// Execute cURL session and get the API response for checking SMS status
									$statusCheckResponse = curl_exec($ch);
									// Check for cURL errors
									if (curl_errno($ch)) {
										file_put_contents($logFileName, "Curl error: " . curl_error($ch) . ". " . PHP_EOL, FILE_APPEND);
									}
									...
									// Decode the response for checking SMS status
									$statusCheckResponseDecoded = json_decode($statusCheckResponse, true);
									file_put_contents($logFileName, $statusCheckResponse . PHP_EOL, FILE_APPEND);
								}
							}
						} catch (Exception $e) {
							// Log any exceptions
						...
						echo "Error fetching contest details";
					}
				} else {
			...
			if (isset($extra_id) && is_numeric($extra_id) && $extra_id != '0') {
				...
				$createContestResponse['status'] = true;
				$createContestResponse['message'] = 'Contest created';
			}
		}

		echo json_encode($createContestResponse);
		die;
	}
...
if (isset($_GET['extrap']) && $_GET['extrap'] == 'settings') {
	...
	if (isset($_POST['submit'])) {
		...
		$new_settings['paystack_public_key'] = mysqli_real_escape_string($db, $_POST['paystack_public_key']);
...
<body>
	...
	<script type="text/javascript">
		(function($) {
			...
			$('#contestForm').submit(function(e) {
				...
				$('#admin_create_contest').text('creating contest...');
				...
				$.ajax({
					...
					success: function(response) {
						if (response.hasOwnProperty('errors')) {
                            ...
                            $('.error_box').text(response['errors'].join(', '));
							$('.error_box').show();
                        } else {
							$('.success_box').text('Contest created');
							$('.error_box').show();
						}
						
						$('#admin_create_contest').text('Save changes');
					},
					...
					if (get['users'].length) {
						...
						for (i = 0; i <= get['users'].length - 1; i++) {
							...
							var result = '' +
								...
								'<div class="users_dash_col users_dash_col_4 users_dash_extra_2">' + get['users'][i].mobile_number + '</div>' +
								'<div class="users_dash_col users_dash_col_5 users_dash_extra_2">' + get['users'][i].regdate + '</div>' ;
								if(get['role_id']==2){
									result = result +'<div class="users_dash_col center users_dash_col_6 users_dash_extra_3">' + option_list + '</div>' ;
								}
								result = result +'</div>';
			...
			$(document).on('click', '.edit_user', function() {
				...
				$.post('request.php', {
					reason: 'get_edit_user',
					id: id
				}, function(get) {
					$('#euser_email').stop().val(get.email);
					$('#euser_name').stop().val(get.name);
					$('#euser_user').stop().val(get.user);
					$('#mobile_number').stop().val(get.mobile_number);
					$('select[name="assignrole"]').val(get.role_id);
					$('.pop_ncat').stop().show();
				}, 'json');
			...
			$(document).on('click', '.export_button', function() {
				...
				$.post('request.php', {
					...
					if (response.success) {
						// Download the file
						var blob = new Blob([response.file_content], {
							type: 'text/csv'
						});
			...
			/* making contest form 
			field compulsory start*/

			

			/* making contest form 
			field compulsory end */
			...
			$(document).on('click', '.pop_ncat_op_submit_su', function() {
				...
				var selectedRole = $('select[name="assignrole"]').val();
					...
					if (get['contests'].length) {
						...
						for (i = 0; i <= get['contests'].length - 1; i++) {
							...
							var result = '' +
								...
								if(get['role_id']==2){
									result = result +	'<div class="edit_contest" style="cursor:pointer;padding-top:10px;padding-bottom:10px;font-size:14px;" data-id="' + get['contests'][i].id + '"><i class="fas fa-pencil-alt"></i>&nbsp;&nbsp;Edit contest</div>' +
					...
					if (get['ratings'].length) {
						...
						for (i = 0; i <= get['ratings'].length - 1; i++) {
							...
							var rating = '' +
								...
								if(get['role_id']==2){
									rating = rating +'<div class="users_dash_col center ratings_dash_col_6 remove_rating red2" data-id="' + get['ratings'][i].id + '">&#10005;&nbsp;</div>' ;
	...
	var submitButton = contestForm.querySelector('[name="submit"]');
    ...
    if (contestForm) {
        contestForm.addEventListener('submit', function(event) {
            ...
            // Collect form data
            var formData = new FormData(contestForm);
			formData.append('submit', 'Save changes');

            // Log FormData elements to the console
            ...
            $.ajax({
                ...
                success: function(response) {
                   ...
                   window.location.href = 'index.php?page=contests';
                },
