...
function sendMail($emailDetails, $toAddresses)
{
	...
	try {
		...
		if (isset($settings['smtp_host']) && $settings['smtp_host'] != '') {
			...
			$mail->Subject = $emailDetails['subject'] . ' - ' . $settings['site_logo'];
			...
			if ($success) {
				echo 'Email sent successfully.' . $emailDetails['to'];
			} else {
...
function sendEmail($to, $name,$subject, $message) 
{
	...
	$mail->CharSet = 'UTF-8';
	$settings = get_settings();
	$result = ['success' => false, 'msg' => '']; // Initialize result array
	try {
		...
		if (isset($settings['smtp_host']) && $settings['smtp_host'] != '') {
			...
			$mail->Subject = $subject . ' - ' . $settings['site_logo'];
			...
			if ($success) {
				...
				$result['msg'] = 'Failed to send email. Error: ' . $mail->ErrorInfo;
...
function get_contest($id = 0)
{
	...
	if (mysqli_num_rows($check_contest)) {
		...
		return array(
			...
			'entry_fee'=> $fetch_contest['entry_fee'],
...
function is_logged()
{
	...
	if (
		isset($_SESSION['_logged']) && $_SESSION['_logged'] == '1' ||
		isset($_SESSION['_logged_admin']) && $_SESSION['_logged_admin'] == '1'
	) {
		return true;
	} else {
		return false;
...
function i_rated($photo_id)
{
	...
	if (is_logged()) {
		...
		$sql = mysqli_query($db, "SELECT * FROM `ratings` WHERE `iduser` = '" . $user_id . "' AND `photo_id` = '" . $photo_id . "' LIMIT 1");
	} else {
...
function is_home()
{
	...
	if (!isset($_GET['profile']) && !isset($_GET['uploader']) && !isset($_GET['my_contests']) && !isset($_GET['create_contest']) && !isset($_GET['purchase']) && !isset($_GET['contact']) && !isset($_GET['forgot']) && !isset($_GET['contests']) && !isset($_GET['photo']) && !isset($_GET['contest']) && !isset($_GET['extra_page']) && !isset($_GET['settings']) && !isset($_GET['ranking']) && !isset($_GET['completed_contests'])) {
		return true;
	}
}
...
function current_page()
{
	...
	if (isset($_GET['completed_contests'])) {
