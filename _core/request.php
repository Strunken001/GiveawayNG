...
if ($reason == 'paynow_joincontest' && isset($_POST['item_id']) && is_numeric($_POST['item_id']) && isset($_SESSION['_logged_id']) && is_numeric($_SESSION['_logged_id'])) {
	...
	$contest_detail = get_contest($item_id);
	$settings = get_settings();
	if (isset($settings['payment_gateway']) && $settings['payment_gateway'] == 'paystack') {


		$return['ptype'] = 'paystack';
		...
		$return['order_id'] = 'GAC-' . rand(11111, 99999);
		$return['currency'] = $settings['paypal_currency'];
		$return['price'] = $contest_detail['entry_fee'];
		...
		$return['unid'] = $unid;
		$return['error'] = 0;
	} elseif (isset($settings['payment_gateway']) && $settings['payment_gateway'] == 'razorpay') {
		...
		$order = $api->order->create(array('amount' => $contest_detail['entry_fee'], 'currency' => $settings['paypal_currency']));
		$orderId = $order['id'];
...
if ($reason == 'paynow_contest' && isset($_POST['item_id']) && is_numeric($_POST['item_id']) && isset($_SESSION['_logged_id']) && is_numeric($_SESSION['_logged_id'])) {
	...
	// This is for new contest payment
	...
	if (isset($settings['payment_gateway']) && $settings['payment_gateway'] == 'paypal') {
...
if ($reason == 'paynow' && isset($_POST['item_id']) && is_numeric($_POST['item_id']) && isset($_SESSION['_logged_id']) && is_numeric($_SESSION['_logged_id'])) {
	...
	if (mysqli_num_rows($sql)) {
		...
		if (isset($settings['payment_gateway']) && $settings['payment_gateway'] == 'paystack') {
			...
			$return['order_id'] = rand(1111, 9999);
			$return['currency'] = $settings['paypal_currency'];
			...
			$unid = md5($_SESSION['_logged_id'] . $item_id . time());
			mysqli_query($db, "INSERT INTO `payments` (`txn_id`,`photo_id`,`item_number`,`payment_gross`,`currency_code`,`payment_status`,`userid`) VALUES ('" . $unid . "','" . $photo_id . "','" . $item_id . "','" . $current_purchase['price'] . "','" . $return['currency'] . "','Open','" . $_SESSION['_logged_id'] . "')");



			// Update Rating and review

			// $sql_s = "SELECT * FROM `content` WHERE `photo` = '".$photo_name."'";

			// $result = mysqli_query($db,$sql_s);

			// $row = mysqli_fetch_array($result);

			// $updatedVotes = $row['rating'] + $votes;
			// $updatedViews = $row['views'] + $views;

			// mysqli_query($db,"UPDATE `content` SET `nr_ratings` = $updatedVotes, `rating` = $updatedVotes, `views` = $updatedViews WHERE `photo` = '".$photo_name."' LIMIT 1");


			// if(is_logged()) {
			// 	mysqli_query($db,"INSERT IGNORE INTO `ratings` (`iduser`,`rate`,`photo_id`,`ip`) VALUES ('".$_SESSION['_logged_id']."','".$votes."','".$photo_id."','".my_ip()."')");
			// } else {
...
if ($reason == 'add_comment' && is_logged() && isset($_POST['comment']) && isset($_POST['photo_id']) && is_numeric($_POST['photo_id'])) {
	...
	if (isset($_SESSION['_logged_id'])) {
		$user_id = $_SESSION['_logged_id'];
		$return['added_by'] = 'user';
	}

	if (isset($_SESSION['_logged_admin'])) {
		$user_id = $_SESSION['_logged_admin'];
		$approved = 1;
		$return['added_by'] = 'admin';
	}


	if (strlen($comment) > 1 && $photo_id) {
		mysqli_query($db, "INSERT INTO `comments` (`photo_id`,`user_id`,`comment`,`approved`) VALUES ('" . $photo_id . "','" . $user_id . "','" . $comment . "','" . $approved . "')");
		$return['error'] = 0;
...
if ($reason == 'comments' && isset($_POST['photo_id']) && is_numeric($_POST['photo_id'])) {
	...
	$photo_id = mysqli_real_escape_string($db, $_POST['photo_id']);
	$sql_comments = mysqli_query($db, "SELECT a.`id`, a.`user_id`, a.`comment`, a.`date`, b.`user`, b.`name`,b.`role_id`, b.`profile_picture` FROM `comments` a INNER JOIN `users` b ON a.`user_id` = b.`id` WHERE a.`approved` = '1' AND a.`photo_id` = '" . $photo_id . "' ORDER BY a.`id` DESC LIMIT 100") or die(mysqli_error($db));

	while ($fetch_comment = mysqli_fetch_array($sql_comments)) {
		$role_name = "";
		if ($fetch_comment['role_id'] == 2)
			$role_name = $fetch_comment['name'] . '<small style="color: rgba(33, 37, 41, 0.75);"> <i class="fa fa-check-circle fa-sm"></i> Super Admin </small>';
		elseif ($fetch_comment['role_id'] == 1)
			$role_name = $fetch_comment['name'] . '<small style="color: rgba(33, 37, 41, 0.75);"> <i class="fa fa-check-circle fa-sm"></i> Admin </small>';
		else
			$role_name = $fetch_comment['name'];
		$return['comments'][] = array(
			'id' => $fetch_comment['id'],
			'name' => $role_name,
			'picture' => $fetch_comment['profile_picture'],
			'date' => Carbon::parse($fetch_comment['date'])->diffForHumans(),
			'comment' => $fetch_comment['comment'],
			'user' => $fetch_comment['user_id'] == '1' ? 'Admin' : $fetch_comment['user'],
		...
		if ($email && $password && strlen($password) > 5) {
			...
			if (mysqli_num_rows($check)) {
				...
				if ($eok == '1') {
					...
					$_SESSION['_logged_role'] = $get['role_id'];
...
if ($reason == 'register') {
	...
	if (isset($_POST['email']) && $_POST['email'] && isset($_POST['name']) && $_POST['name'] && isset($_POST['password']) && $_POST['password'] && isset($_POST['repeat_password']) && $_POST['repeat_password']) {
		...
		$category = mysqli_real_escape_string($db, $_POST['category']);
		$phone_number = mysqli_real_escape_string($db, $_POST['fullPhoneNumber']);
		...
		if ($return['error'] == '0') {
			...
			mysqli_query($db, "INSERT INTO `users` (`confirmed`,`mobile_number`,`email`,`name`,`password`,`user`,`category`) VALUES ('" . $confirmed . "','" . $phone_number . "','" . $email . "','" . $name . "','" . hash('sha512', $password) . "','" . $user . "','" . $category . "')") or die(mysqli_error($db));
			...
			if ($confirmed == '0') {
				...
				if (isset($settings['smtp_host']) && $settings['smtp_host'] != '') {
					...
					$mail->Body    = 'Hello ' . $name . ',<br><br>To confirm your e-mail address please go to: <a href="' . $settings['site_url'] . 'index.php?confirm_email=' . md5($my_id) . '" class="btn btn-success" style="background:green; color:#fff;padding:10px; text-decoration:none;">Click Here</a>';
					$mail->IsHTML(true);
					$mail->send();
				} else {
					...
					$mail->isHTML(true);
					$mail->Subject = 'Confirm e-mail - ' . $settings['site_logo'];
					$mail->Body    = 'Hello ' . $name . ',<br><br>To confirm your e-mail address please go to: <a href="' . $settings['site_url'] . 'index.php?confirm_email=' . md5($my_id) . '" class="btn btn-success">' . $settings['site_url'] . 'index.php?confirm_email=' . md5($my_id) . '</a>';
					$mail->send();
				}
			}
	...
	if (mysqli_num_rows($sql_count)) {
		...
		if (!isset($settings['content_ratemode']) || (isset($settings['content_ratemode']) && $settings['content_ratemode'] == '0')) {
			...
			if ($fetch_count['rating'] && $fetch_count['total_ratings']) {
				...
				if (strlen($new_rating) == 1) {
					$new_rating = $new_rating . '.00';
				}
				if (strlen($new_rating) == 3) {
					$new_rating = $new_rating . '0';
				}
				if (strlen($new_rating) > 4) {
					$new_rating = substr($new_rating, 0, 4);
				}
			} else {
				$new_rating = '0.00';
				$total_ratings = 0;
		...
		// Select current ratings and views
		$sql_s1 = "SELECT * FROM `content` WHERE `id` = '" . $photo_id . "'";

		$result = mysqli_query($db, $sql_s1);

		$row = mysqli_fetch_array($result);



		$updatedVotes = $row['rating'] + $rate;



		mysqli_query($db, "UPDATE `content` SET `nr_ratings` = '" . $updatedVotes . "', `rating` = '" . $updatedVotes . "' WHERE `id` = '" . $photo_id . "' LIMIT 1");
